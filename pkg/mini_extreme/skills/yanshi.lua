...
yanshi:addEffect('active', {
  can_use = function(self, player)
    return player:usedSkillTimes(yanshi.name, Player.HistoryPhase) == player:getMark("_mini_yanshi-phase")
  end,
  target_num = 0,
  card_num = function(self, player)
    return player:usedSkillTimes(yanshi.name, Player.HistoryPhase) > 0 and 1 or 0
  end,
  card_filter = function(self, player, to_select, selected_cards)
    return #selected_cards < (player:usedSkillTimes(yanshi.name, Player.HistoryPhase) > 0 and 1 or 0)
  ...
  on_use = function(self, room, effect)
    ...
    if #effect.cards > 0 then
      room:throwCard(effect.cards, yanshi.name, player, player)
      if player.dead then return end
    end
    player:drawCards(1, yanshi.name, choice == "Bottom" and "bottom" or "top", "@@mini_yanshi-inhand-phase")
  end,
})

yanshi:addEffect(fk.PreCardUse, {
