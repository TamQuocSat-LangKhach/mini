...
Fk:loadTranslationTable{
  ...
  [':mini_taoni'] = '出牌阶段开始时，你可失去任意点体力，摸等量的牌，然后令至多X名没有“讨逆”的其他角色各获得1枚“讨逆”（X为你以此法失去的体力值），此回合你的手牌上限为你的体力上限。',
  ['$mini_taoni1'] = '欲立万丈之基，先净门庭之度。',
  ['$mini_taoni2'] = '扫定四野，百姓自当归附。',
}

taoni:addEffect(fk.EventPhaseStart, {
  anim_type = "drawcard",
  can_trigger = function(self, event, target, player)
    return target == player and player:hasSkill(taoni) and player.phase == Player.Play and player.hp > 0
  end,
  ...
  on_use = function(self, event, target, player)
    local num = tonumber(event:getCostData(self)) ---@type integer
    local room = player.room
    room:loseHp(player, num, taoni.name)
    if player.dead then return end
    room:drawCards(player, num, taoni.name)
    if player.dead then return end
