local miniGanlu = fk.CreateSkill {
  name = "mini__ganlu"
}
...
miniGanlu:addEffect("active", {
  prompt = "#mini__ganlu-active",
  anim_type = "control",
  card_num = 1,
  target_num = 0,
  can_use = function(self, player)
    return player:usedSkillTimes(miniGanlu.name, Player.HistoryPhase) == 0
  end,
  ...
  on_use = function(self, room, effect)
    ---@type string
    local skillName = miniGanlu.name
    ...
    if player:isAlive() then
      ...
      if choice == "mini__ganlu_move" then
        ...
        room:moveCardIntoEquip(player, put, skillName, false, player)
      end
    end
  end,
})
