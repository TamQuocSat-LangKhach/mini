...
local sixiao = fk.CreateTriggerSkill{
  ...
  on_use = function(self, event, target, player, data)
    ...
    if event == fk.GameStart then
      ...
      local allcards = player:getCardIds("h")
      local cards = table.filter(allcards, function (id)
        local c = Fk:getCardById(id)
        return not target:prohibitUse(c) and Exppattern:Parse(data.pattern):match(c)
      end)
      -- copy jixiang
